...
<html>
	...
	<body>
		...
		<div class = "act"><!-- Форма поиска клиента-->
			<form method = "post">
				<input required type="text" class="form-control" name="nomer_cheta"  id="nomer_pas" placeholder="Введите номер счета" autocomplete="off" list = "Num_chet"/><br>
			...
			<?php
				if($_SESSION["user_date"] != null){//Вывод данных о клиенте
					echo'
						...
						<label>Баланс RUB: '.$_SESSION["user_date"]["RUB"].'</label><br/>
						<label>Баланс USD: '.$_SESSION["user_date"]["USD"].'</label><br/>
						<label>Баланс EUR: '.$_SESSION["user_date"]["EUR"].'</label><br/>
						<label>Баланс CHF: '.$_SESSION["user_date"]["CHF"].'</label><br/>
						<label>Баланс CZK: '.$_SESSION["user_date"]["CZK"].'</label><br/>
		...
		?>	
		<form method = "post" action = " " class = "abgreyd"><!-- Форма вывода отчета-->
